    ...
    <section className="relative min-h-screen flex flex-col justify-center items-center overflow-hidden py-20">
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
         ...
         <svg className="absolute inset-0 w-full h-full opacity-40" viewBox="0 0 1200 800" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* Left to Center (Converging on Photo area roughly at 850, 400) */}
            <PipelinePath d="M -100 100 C 200 100 400 200 800 380" duration={6} delay={0} />
            <PipelinePath d="M -100 300 C 150 300 350 350 820 400" duration={7} delay={1.5} />
            <PipelinePath d="M -100 500 C 200 500 400 500 810 420" duration={5} delay={3} />
            <PipelinePath d="M -100 700 C 250 700 450 650 830 410" duration={8} delay={0.5} />
            
            {/* Right to Center */}
            <PipelinePath d="M 1300 150 C 1000 150 900 250 850 350" duration={7.5} delay={4} />
            <PipelinePath d="M 1300 350 C 1100 350 950 400 860 380" duration={6} delay={1} />
            <PipelinePath d="M 1300 550 C 1100 550 1000 520 870 400" duration={5.5} delay={2.5} />
            
            {/* Top to Center */}
            <PipelinePath d="M 400 -100 C 400 100 600 200 810 370" duration={9} delay={5} />
            <PipelinePath d="M 800 -100 C 800 100 850 200 840 375" duration={8.5} delay={3.5} />
         </svg>
      </div>
               ...
               >
                 {[...Array(2)].map((_, i) => (
                   <React.Fragment key={i}>
                     <p className="flex items-center gap-2"><span className="text-emerald-500">✓</span> [SYSTEM] Production cluster initialized.</p>
                     <p className="flex items-center gap-2"><span className="text-indigo-500">→</span> [AUTH] OAuth2 handshake successful.</p>
                     <p className="flex items-center gap-2"><span className="text-indigo-500">→</span> [DB] PostgreSQL connection pool active.</p>
                     <p className="flex items-center gap-2"><span className="text-fuchsia-500">⚡</span> [CI/CD] Deployment lint gate passed.</p>
                     <p className="flex items-center gap-2"><span className="text-indigo-500">→</span> [ARK] Streaming vector-clock: 0x4f3</p>
                     <p className="flex items-center gap-2"><span className="text-indigo-500">→</span> [CACHE] Redis hit rate: 99.2%</p>
                     <p className="flex items-center gap-2"><span className="text-indigo-500">→</span> [NET] Traffic routed to Vercel Edge.</p>
                     <p className="flex items-center gap-2"><span className="text-indigo-500">→</span> [OPS] 4 worker nodes at 12% load.</p>
                     <p className="flex items-center gap-2"><span className="text-emerald-500">✓</span> [SYNC] Real-time state synchronized.</p>
                     <p className="flex items-center gap-2"><span className="text-fuchsia-500">⚡</span> [API] Endpoint /v1/health: 200 OK</p>
                   </React.Fragment>
                 ))}
               </motion.div>
...
function PipelinePath({ d, duration, delay }) {
  return (
    <g>
      <motion.path
        d={d}
